...
parser.add_argument( "--db",  help='Execução completa no banco de dados', action='store_true' )
...
def import_query( path:str, **kwargs ):
    '''Função para importar uma query de um diretório...'''
    with open( path, 'r', **kwargs ) as file_open:
        query = file_open.read()
    return query

print("\nAbrindo conexão com o banco de dados... ", end="")
...
if args.db:
    ...
    print("ok.")
    print("Executando query... ", end="")
    df_candidatos_bens = pd.read_sql_query( query_bens_candidatos, con)
    print("ok.")

    print("Trantando os dados no pandas... ", end="")
    result = (df_candidatos_bens.sort_values( by='total_declarado',
                                            ascending=False ) 
                                .head(args.top) )[['nome', 'total_declarado']]
    print("ok.")

if args.save:
    print("Salvando no banco... ")
    result.to_sql( 'tb_resultado_bens', con, index=False, if_exists='replace' )
    print("ok.")
